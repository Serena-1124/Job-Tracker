...
const PositionDistributionChart: React.FC<PositionDistributionChartProps> = ({ deliveries }) => {
  const chartData = useMemo(() => {
    const positionCount: Record<string, number> = {};

    deliveries.forEach(d => {
      if (d.positionTypeName) {
        positionCount[d.positionTypeName] = (positionCount[d.positionTypeName] || 0) + 1;
      }
    });
